...
def webhook(request, *args, **kwargs):
    ...
    if 'notificationItems' in event_json:
        for notification_item in event_json['notificationItems']:
            ...
            reference = notification_item['merchantReference'].split('-')
            try:
                if reference[2] == 'R':
    ...
    def get(self, request, *args, **kwargs):
        ...
        if 'action' in payment_info:
            ctx = {
                ...
                'environment': 'test' if self.order.testmode else self.order.event.settings.prod_env,
                'originKey': prov._get_originKey('test' if self.order.testmode else self.order.event.settings.prod_env),
